...
// Ideias pra pagar menos imposto — mesmo esquema do crédito (reaproveitado
// pelos 3 módulos), mas aqui o cuidado é redobrado: só pode sugerir
// PLANEJAMENTO TRIBUTÁRIO LEGAL (elisão fiscal — enquadramento, dedução
// correta, benefício previsto em lei), NUNCA sonegação/caixa
// dois/nota fria/omitir receita ou qualquer manobra ilegal — isso fica
// explícito no prompt, com instrução de recusar educadamente se a
// pessoa pedir claramente algo ilegal. Sempre em cima de dado real
// (nunca invenção) e sempre deixando claro que não substitui um
// contador de verdade olhando o caso específico.
const _CONTEXTO_IMPOSTOS = {
  lar: 'ajudando uma PESSOA/FAMÍLIA a pensar no Imposto de Renda pessoa física dela. Ideias legais relevantes: comparar declaração completa x simplificada (qual desconta mais), deduções legais — dependentes, despesas médicas e plano de saúde (sem limite, mas precisa de recibo/nota), educação (com limite anual por pessoa), contribuição pra previdência privada PGBL (até 12% da renda bruta tributável, só faz sentido pra quem já contribui pro INSS ou regime próprio), doações a fundos do idoso/da criança/cultura que geram abatimento direto no imposto devido — sempre guardando recibo/comprovante de tudo.',
  agro: 'ajudando um PRODUTOR RURAL a pensar nos impostos da propriedade dele. Ideias legais relevantes: manter o livro caixa do produtor rural em dia (permite abater do IR as despesas de custeio e investimento, inclusive compra de máquina/benfeitoria), entender o Funrural (retenção sobre a receita bruta na venda, com alíquota que muda conforme a situação), avaliar se compensa migrar pra pessoa jurídica (agroindústria) quando o porte cresce, e depreciar máquinas/benfeitorias como despesa ano a ano em vez de só no ano da compra.',
  empresa: 'ajudando uma pequena/média EMPRESA a pensar nos impostos dela. Ideias legais relevantes: conferir se o regime tributário (Simples Nacional, Lucro Presumido ou Lucro Real) ainda é o mais vantajoso pro faturamento atual — inclusive ficar de olho pra não estourar sem querer o teto de faixa do Simples no fim do ano, equilibrar a proporção entre pró-labore e distribuição de lucros dos sócios (tributação é diferente pra cada um), guardar nota fiscal de toda despesa dedutível pra não pagar imposto em cima de receita que já teve custo, e considerar o timing de compras/investimentos grandes pra aproveitar melhor o regime do ano.'
};

async function _sugerirReducaoImpostosResposta(modulo, resumoDados, mensagem) {
  const promptImpostos = `Você é o Zeca, o assistente do GuiaZap, e a pessoa pediu ideias pra pagar menos imposto / reduzir impostos. Você está ${_CONTEXTO_IMPOSTOS[modulo]}

Dados financeiros reais dela: ${resumoDados}

REGRA MAIS IMPORTANTE: você só pode sugerir PLANEJAMENTO TRIBUTÁRIO 100% LEGAL (elisão fiscal — enquadramento certo, dedução prevista em lei, benefício fiscal de verdade). NUNCA sugira nem ajude com sonegação, caixa dois, nota fria, omitir receita, "não declarar", laranja, ou qualquer manobra ilegal — se pelo pedido da pessoa parecer que é isso que ela quer, recuse educadamente essa parte especificamente (sem sermão longo) e ofereça só o caminho legal.

Sua resposta deve: (1) comentar bem brevemente a situação com base SÓ nos dados acima (nunca invente número que não está aí), (2) sugerir 2-3 ideias LEGAIS de planejamento tributário que fazem mais sentido pro perfil dela, e (3) terminar deixando claro, no seu jeito de falar (frase curta, não é aviso jurídico formal), que você não é contador, que cada uma dessas ideias tem regra própria que muda caso a caso, e que o certo é levar isso pro contador dela confirmar antes de fazer qualquer coisa. Responda em português, tom direto e prático, no máximo 6-7 frases curtas.

Responda APENAS com um JSON válido: {"resposta": "sua resposta aqui"}`;

  const iaImpostos = await chamarIABarata(promptImpostos, mensagem, 400, true);
  if (!iaImpostos.ok || !iaImpostos.json || !iaImpostos.json.resposta) {
    return _mensagemFalhaIA(iaImpostos);
  }
  return iaImpostos.json.resposta;
}
...
// dá pra trocar direto no Netlify sem precisar mexer em código.
// "gemini-2.0-flash-lite" (valor antigo daqui) ERA da mesma família do
// "gemini-2.0-flash" que o Google confirmou ter DESCONTINUADO em
// produção (19/09/2026, 404 pedindo pra trocar pra "gemini-3.6-flash" —
// ver comentário do GEMINI_MODELO_VIDEO logo abaixo). O vídeo foi
// corrigido na hora, mas a análise de imagem ficou passando batido
// (nenhuma tela dava erro claro, só a resposta genérica de "não
// consegui olhar essa imagem", então demorou pra alguém perceber).
// Trocado pro mesmo modelo já confirmado funcionando.
...
- REGRA GERAL DE TEMA SENSÍVEL (vale só pro texto que você mesmo gera no campo "resposta" do tipo "resposta" — tipo "geral" NÃO gera resposta aqui, ver regra própria dele acima, o aviso de tema sensível dele fica no prompt separado que trata esse tipo depois): sempre que a pergunta for sobre saúde/remédio/sintoma, gravidez, dívida em atraso/nome sujo, demissão/direito trabalhista, doação, investir em energia solar/painel solar, conteúdo adulto, criança/adolescente, PCD ou aprendiz (contratação), FGTS/benefício do INSS ou governo, terra/INCRA/CAR/regularização de propriedade, agrotóxico/defensivo agrícola, ou vacina/remédio de animal — COMECE a resposta com uma frase curta e natural (nunca um aviso jurídico formal, nada de letra miúda) deixando claro que é só uma sugestão, que você não é médico/advogado/contador/veterinário/técnico daquela área, e que a decisão final é da pessoa e do profissional que ela procurar. Isso é ALÉM do aviso que já existe embutido nos prompts de crédito e de imposto (que já cobrem esse caso à parte) — aqui é pra cobrir os outros temas pesados que caem direto no tipo "resposta" sem ter um prompt dedicado. Se, em qualquer ponto da conversa, a pessoa mencionar vontade de se machucar ou de morrer: não dê nenhum detalhe prático sobre método nenhum, oriente com cuidado a ligar pro CVV (188, atendimento gratuito 24h) ou procurar o CAPS/pronto-socorro mais perto, e pare de aprofundar o assunto técnico que ela tinha perguntado antes.
...
- tipo "lar": quando a pessoa fala de FINANÇAS/BENS PESSOAIS/DOMÉSTICOS (não de empresa) — registrar receita/despesa da casa (ex: "gastei 200 na conta de luz", "recebi meu salário de 3000"), pedir resumo (ex: "quanto gastei com a casa esse mês?", "separa minhas despesas por categoria"), cadastrar um bem (ex: "comprei um carro por 60 mil", "tenho um apartamento que vale 300 mil"), pedir orientação sobre empréstimo/financiamento/linha de crédito PESSOAL (ex: "preciso pegar um empréstimo", "quais linhas de crédito eu consigo?") — acao_lar "sugerir_credito" — ou pedir ideia pra pagar menos Imposto de Renda pessoal (ex: "como eu pago menos IR?", "tem jeito de abater mais no imposto?") — acao_lar "sugerir_reducao_impostos". Nunca confunde com "financeiro" (que é da EMPRESA da pessoa) — se não estiver claro se é pessoal ou da empresa, pergunta antes (tipo "resposta").
- tipo "agro": quando a pessoa fala de PROPRIEDADE RURAL/produção agrícola — registrar receita/despesa (ex: "gastei 500 em adubo", "vendi 200 sacas de milho por X"), pedir resumo (ex: "quanto custou minha safra de milho?", "resumo financeiro da propriedade"), cadastrar um bem da propriedade (ex: "comprei um trator", "tenho 40 cabeças de gado") — acao_agro "cadastrar_patrimonio" — registrar uma colheita/produção (ex: "colhi 300 sacas de milho", "produção de 5 toneladas de soja") — acao_agro "registrar_producao" — criar uma nova safra/plantio (ex: "comecei a safra de soja", "nova safra de milho verão") — acao_agro "cadastrar_safra" — criar um novo talhão/área da propriedade (ex: "cadastra o talhão 3", "tenho uma área nova de 10 hectares") — acao_agro "cadastrar_talhao" — pedir orientação sobre empréstimo/financiamento/linha de crédito RURAL (ex: "preciso de crédito pra plantar", "quais linhas de crédito rural existem?") — acao_agro "sugerir_credito" — ou pedir ideia pra pagar menos imposto na propriedade (ex: "como reduzo o imposto da minha propriedade?", "tem jeito de pagar menos Funrural?") — acao_agro "sugerir_reducao_impostos". Se mencionar uma cultura/safra específica (pra lançar, consultar ou produzir), preenche safra_agro.
- tipo "empresa": quando a pessoa (dona de empresa, logada) já usa o painel completo da Empresa (clientes, fornecedores, pedidos, estoque, equipe) e quer lançar uma receita/despesa nesse caixa mais completo (acao_empresa "lancar" — só usa esse tipo em vez de "financeiro" se a pessoa mencionar algo do painel completo, tipo cliente/fornecedor/pedido/estoque/equipe, OU já tiver usado a Empresa antes na conversa; senão prefere "financeiro" que é mais simples e já é o padrão histórico), pergunta como está indo o negócio no geral (acao_empresa "resumo"), o que mais vende (acao_empresa "mais_vendidos"), o que tá acabando no estoque (acao_empresa "estoque_baixo"), pede uma opinião/conselho/sugestão sobre a empresa (ex: "como posso vender mais?", "o que você acha do meu negócio esse mês?", "tenho algum produto parado?") — acao_empresa "conselho" — pede orientação sobre empréstimo/financiamento/linha de crédito PRA EMPRESA (ex: "preciso de um empréstimo pro negócio", "quais linhas de crédito eu consigo?") — acao_empresa "sugerir_credito" — ou pede ideia pra pagar menos imposto na empresa (ex: "como pago menos imposto?", "meu regime tributário tá certo?", "tem manobra legal pra reduzir imposto?") — acao_empresa "sugerir_reducao_impostos".
...
- Colaboradores ativos: ${dadosEmp.colaboradoresAtivos}${dadosEmp.folhaSalarial ? `, folha salarial: ${fmtEmp(dadosEmp.folhaSalarial)}` : ''}`;
      ...
      if (decisao.acao_empresa === 'sugerir_reducao_impostos') {
        const linhasCategoriaImp = Object.entries(dadosEmp.porCategoriaDespesa).sort((a, b) => b[1] - a[1]).map(([cat, v]) => `${cat}: ${fmtEmp(v)}`).join(', ') || 'nenhuma';
        const resumoImpEmp = `Empresa "${empresaEmp.name}" (${periodoEmp.rotulo}): Receitas ${fmtEmp(dadosEmp.totalReceitas)}, Despesas ${fmtEmp(dadosEmp.totalDespesas)} (por categoria: ${linhasCategoriaImp}), Saldo ${fmtEmp(dadosEmp.totalReceitas - dadosEmp.totalDespesas)}, Colaboradores ativos: ${dadosEmp.colaboradoresAtivos}${dadosEmp.folhaSalarial ? ` (folha ${fmtEmp(dadosEmp.folhaSalarial)})` : ''}.`;
        const respostaImpEmp = await _sugerirReducaoImpostosResposta('empresa', resumoImpEmp, mensagem);

        let conversaIdImpEmp = null;
        if (usaMemoria) conversaIdImpEmp = await salvarTrocaDeMensagens(usuarioIdChat, conversaId, mensagem, respostaImpEmp);
        return { statusCode: 200, body: JSON.stringify({ resposta: respostaImpEmp, conversaId: conversaIdImpEmp }) };
      }
    ...
    if (decisao.tipo === 'lar') {
      ...
      if (decisao.acao_lar === 'sugerir_reducao_impostos') {
        // IR pessoa física se pensa no ano inteiro, não no mês —
        // _periodoFinanceiroZeca não tem opção de "ano inteiro" (mesmo
        // caso já resolvido em sugerir_credito/agro), então monta o
        // intervalo na mão aqui, igual ao preparar-ir-zeca.js.
        const anoImpLar = new Date().getFullYear();
        const lancImpLarResp = await fetch(
          `${SUPABASE_URL}/rest/v1/lar_lancamentos?user_id=eq.${usuarioIdChat}&data=gte.${anoImpLar}-01-01&data=lte.${anoImpLar}-12-31&select=tipo,valor,categoria`,
          { headers }
        );
        const lancImpLar = lancImpLarResp.ok ? await lancImpLarResp.json() : [];
        const fmtImpLar = v => `R$ ${Number(v).toFixed(2).replace('.', ',')}`;
        const receitaImpLar = lancImpLar.filter(l => l.tipo === 'receita').reduce((s, l) => s + Number(l.valor), 0);
        const despesaImpLar = lancImpLar.filter(l => l.tipo === 'despesa').reduce((s, l) => s + Number(l.valor), 0);
        const porCategoriaImpLar = {};
        lancImpLar.filter(l => l.tipo === 'despesa' && l.categoria).forEach(l => { porCategoriaImpLar[l.categoria] = (porCategoriaImpLar[l.categoria] || 0) + Number(l.valor); });
        const linhasCategoriaImpLar = Object.entries(porCategoriaImpLar).sort((a, b) => b[1] - a[1]).map(([cat, v]) => `${cat}: ${fmtImpLar(v)}`).join(', ') || 'nenhuma';
        const resumoImpLar = lancImpLar.length
          ? `Finanças pessoais de ${anoImpLar}: Receitas ${fmtImpLar(receitaImpLar)}, Despesas ${fmtImpLar(despesaImpLar)} (por categoria: ${linhasCategoriaImpLar}).`
          : `Ainda não tem lançamento pessoal registrado esse ano (o Zeca não tem visão de outras rendas/gastos além do que já foi lançado aqui).`;
        const respostaImpLar = await _sugerirReducaoImpostosResposta('lar', resumoImpLar, mensagem);

        let conversaIdImpLar = null;
        if (usaMemoria) conversaIdImpLar = await salvarTrocaDeMensagens(usuarioIdChat, conversaId, mensagem, respostaImpLar);
        return { statusCode: 200, body: JSON.stringify({ resposta: respostaImpLar, conversaId: conversaIdImpLar }) };
      }
    ...
    if (decisao.tipo === 'agro') {
      ...
      if (decisao.acao_agro === 'sugerir_reducao_impostos') {
        const anoImpAgro = new Date().getFullYear();
        const lancImpAgroResp = await fetch(
          `${SUPABASE_URL}/rest/v1/agro_lancamentos?propriedade_id=eq.${propriedade.id}&data=gte.${anoImpAgro}-01-01&data=lte.${anoImpAgro}-12-31&select=tipo,valor,categoria`,
          { headers }
        );
        const lancImpAgro = lancImpAgroResp.ok ? await lancImpAgroResp.json() : [];
        const fmtImpAgro = v => `R$ ${Number(v).toFixed(2).replace('.', ',')}`;
        const receitaImpAgro = lancImpAgro.filter(l => l.tipo === 'receita').reduce((s, l) => s + Number(l.valor), 0);
        const despesaImpAgro = lancImpAgro.filter(l => l.tipo === 'despesa').reduce((s, l) => s + Number(l.valor), 0);
        const porCategoriaImpAgro = {};
        lancImpAgro.filter(l => l.tipo === 'despesa' && l.categoria).forEach(l => { porCategoriaImpAgro[l.categoria] = (porCategoriaImpAgro[l.categoria] || 0) + Number(l.valor); });
        const linhasCategoriaImpAgro = Object.entries(porCategoriaImpAgro).sort((a, b) => b[1] - a[1]).map(([cat, v]) => `${cat}: ${fmtImpAgro(v)}`).join(', ') || 'nenhuma';
        const resumoImpAgro = lancImpAgro.length
          ? `Propriedade "${propriedade.nome}" (ano ${anoImpAgro}): Receitas ${fmtImpAgro(receitaImpAgro)}, Despesas ${fmtImpAgro(despesaImpAgro)} (por categoria: ${linhasCategoriaImpAgro}).`
          : `Ainda não tem lançamento da propriedade "${propriedade.nome}" registrado esse ano.`;
        const respostaImpAgro = await _sugerirReducaoImpostosResposta('agro', resumoImpAgro, mensagem);

        let conversaIdImpAgro = null;
        if (usaMemoria) conversaIdImpAgro = await salvarTrocaDeMensagens(usuarioIdChat, conversaId, mensagem, respostaImpAgro);
        return { statusCode: 200, body: JSON.stringify({ resposta: respostaImpAgro, conversaId: conversaIdImpAgro }) };
      }
...
Se a pergunta for sobre saúde/remédio/sintoma, gravidez, dívida em atraso, demissão/direito trabalhista, doação, investir em energia solar, conteúdo adulto, criança/adolescente, PCD ou aprendiz (contratação), FGTS/benefício, terra/INCRA/CAR, agrotóxico, ou vacina/remédio de animal: comece a resposta com uma frase curta e natural dizendo que é só uma sugestão, que você não é médico/advogado/contador/veterinário/técnico daquela área, e que a decisão final é da pessoa e do profissional que ela procurar. Se a pessoa mencionar vontade de se machucar ou de morrer: não dê detalhe prático de método nenhum, oriente a ligar pro CVV (188, gratuito 24h) ou procurar o CAPS/pronto-socorro mais perto, e pare por aí.
