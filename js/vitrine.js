...
async function _processarComandoVozVitrineInterno(transcricao){
  ...
  const pareceDesativarSomenteVoz = textoNormalizadoDestravar.includes('desativar') && textoNormalizadoDestravar.includes('voz');
  if(_modoSomenteVozAtivoVitrine && (pareceDesativarSomenteVoz || textoNormalizadoDestravar.includes('destravar'))){
    await processarComandoDesativarSomenteVoz(transcricao);
    return;
  }
  ...
  if(typeof verificarNavegacaoUniversalPorVoz === 'function'){
    const destinoUniversal = verificarNavegacaoUniversalPorVoz(textoNormalizado, 'vitrine.html');
    if(destinoUniversal){
      localStorage.setItem('retomarModoVozAoCarregar', '1');
      falarVozVitrine(destinoUniversal.fala);
  ...
  if(textoNormalizado === 'parar'){
    falarVozVitrine('Ok. Modo voz continua ativo, pode pedir outra coisa.');
    return;
  }
