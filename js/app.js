...
async function loadEntries(){
  ...
  if(abrirParam === 'configAtendimento'){
    window.history.replaceState({}, '', window.location.pathname);
    setTimeout(() => { if(typeof abrirConfigAtendimentoDireto === 'function') abrirConfigAtendimentoDireto(); }, 600);
  }
...
// Versão "por toque" do mesmo fluxo de configurar atendimento — usada
// pelo botão visível (não precisa de comando de voz pra achar essa tela)
async function abrirConfigAtendimentoDireto(){
  if(!currentUser){ alert('Você precisa estar logado.'); return; }
  const minhas = minhasEmpresasVendasParaVoz();

  if(minhas.length === 0){
    alert('Você não tem uma empresa com Pacote Vendas — esse recurso é exclusivo desse pacote.');
    return;
  }
  if(minhas.length === 1){
  ...
  // Mais de uma empresa com Pacote Vendas — pergunta qual, de forma simples
  const nomes = minhas.map((e, i) => `${i + 1}. ${e.name}`).join('\n');
  const escolha = prompt(`Você tem mais de uma empresa com Pacote Vendas. Digite o número:\n${nomes}`);
  const indice = parseInt(escolha, 10) - 1;
  if(minhas[indice]) await abrirConfigAtendimento(minhas[indice].id);
}
...
async function processarComandoVozIndex(transcricao){
  ...
  if(typeof verificarNavegacaoUniversalPorVoz === 'function'){
    ...
    if(destinoUniversal){
      irPara(destinoUniversal.url, destinoUniversal.fala);
      return;
    }
  }
  ...
  if(!pareceComandoComplexo && termoBuscaLocal.length >= 2 && termoBuscaLocal.length <= 60){
    ...
    const campoCidade = document.getElementById('gz-localidade-busca');
    const campoBairro = document.getElementById('filter-bairro');
