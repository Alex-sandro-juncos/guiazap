...
// página atual.
//
// IMPORTANTE: esse nome de arquivo é só pra ISSO (decidir se é pedido de
// navegação pra outra página) — não reaproveitar pra outros propósitos,
// senão quebra silenciosamente todo mundo que já chama esse endpoint
// esperando esse formato específico de resposta ({ pagina, resposta_falada }).
//
// Usa os mesmos helpers de IA barata (Gemini Flash-Lite → Claude Haiku) e
// cache de comandos de voz que o resto do projeto já usa, pra não gastar
// caro nem repetir chamada pra frase que já foi vista antes.
//
...
const ESCOPO = 'navegacao-universal';
...
async function estourouLimite(ip){
  ...
  const chave = 'interpretar-navegacao-universal-voz:' + ip;
...
- meus-pedidos.html: histórico de compras da própria pessoa (como cliente, não como empresa)
...
- mapa.html: mapa com os profissionais e empresas perto da pessoa
- agenda.html: agenda de contatos salvos no Papo
- chamar-frete.html: pedir um frete/corrida/motoboy pra buscar ou entregar algo
- corridas.html: "GuiaCorridas", tela do entregador pra ver e aceitar corridas
- banco-entregadores.html: lista de entregadores/motoboys disponíveis pra contratar direto
- talentos.html: banco de talentos, currículos de candidatos disponíveis pra empresa contratar
- feed.html: feed de novidades das empresas que a pessoa segue
- videos.html: vídeos publicados pelas empresas
...
exports.handler = async function (event) {
  try {
    ...
    if (!ANTHROPIC_API_KEY && !process.env.GEMINI_API_KEY) {
      return { statusCode: 500, body: JSON.stringify({ error: 'nenhuma chave de IA configurada' }) };
    }
    ...
    const textoNorm = normalizarTexto(texto + '|' + (paginaAtual || ''));
    const cache = await buscarCache(ESCOPO, textoNorm);
    if (cache) {
      return { statusCode: 200, body: JSON.stringify(cache) };
    }
    await salvarPending(ESCOPO, textoNorm, texto);
